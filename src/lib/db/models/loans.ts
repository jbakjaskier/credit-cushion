...
export function isDbWriteOperationErrorResult(
  input: DbWriteOperationErrorResult | DbWriteOperationSuccessResult
): input is DbWriteOperationErrorResult {
  return input.mode === "error";
}

export function isDbFetcherError(
  input: DbFetcherError | Loan | null | Template | Loan[]
): input is DbFetcherError {
  if (input === null) {
    return false;
  }
